...
class SimulateFunc(torch.autograd.Function):
    ...
    @staticmethod
    def get_envs_with_high_contact_force(contact_f, link_idx, force_threshold, num_envs, link_count, envs_exclude=None):
        """
        Extract environment indices that have contact force greater than threshold for a specific link.

        Args:
            contact_f: Tensor of shape (total_links, 6) where total_links = num_envs * link_count
            link_idx: Index of the specific link to check (0 to link_count-1)
            force_threshold: Force threshold to compare against
            num_envs: Number of environments
            link_count: Number of links per environment
            envs_exclude: Optional tensor of environment indices to exclude from results

        Returns:
            env_indices: Tensor of environment indices with high contact force (excluding envs_exclude)
        """
        # Reshape contact_f to (num_envs, link_count, 6)
        contact_f_reshaped = contact_f.view(num_envs, link_count, 6)

        # Extract contact forces for the specific link across all environments
        link_contact_forces = contact_f_reshaped[:, link_idx, :]  # Shape: (num_envs, 6)

        # Compute force norm (only use force part, last 3 components)
        force_components = link_contact_forces[:, 3:]  # Shape: (num_envs, 3)
        force_norms = torch.norm(force_components, dim=1)  # Shape: (num_envs,)

        # Find environments where force norm exceeds threshold
        high_force_mask = force_norms > force_threshold  # Shape: (num_envs,)

        # Exclude specified environments if provided
        if envs_exclude is not None and len(envs_exclude) > 0:
            # Create exclusion mask
            exclude_mask = torch.ones(num_envs, dtype=torch.bool, device=contact_f.device)
            exclude_mask[envs_exclude] = False
            # Apply exclusion mask
            high_force_mask = high_force_mask & exclude_mask

        # Get environment indices
        env_indices = torch.nonzero(high_force_mask).squeeze(-1)  # Shape: (num_high_force_envs,)
    ...
    def forward(
        ...
        for i in range(substeps):
            ...
            if bundle_info is not None and bundle_info[0]:
                # Check if force is beyond a certain value. If it is then we do bundling until force goes below a threshold.
                # Keep track of which environments are in contact
                num_samples = bundle_info[4]
                sigma = bundle_info[5]
                noise_idx = bundle_info[2]
                force_threshold = 1e3  # TODO: Make this a parameter
...
class SemiImplicitIntegrator:
    ...
    def forward(
        ...
        if dflex.config.no_grad:
            ...
            # contact_forces.append(state_out.contact_f.clone()[-1])
            # if env_ids_in_contact.numel() > 0:
            if False:
                # num_steps_in_contact.append(1)
            ...
            # print()
            # for i, f in zip(num_steps_in_contact, contact_forces):
            #     print(i)
            #     print(torch.norm(f[3:]))
            #     print()
            # print("len num_steps_in_contact")
            # print(len(num_steps_in_contact))
    ...
    def _simulate_bundle(self, tape, model, state_in, state_out, dt, bundle_info, envs_to_bundle, num_bundle_steps,
        ...
        with dflex.util.ScopedTimer("simulate_bundle", False):
            ...
            bundle_joint_qd_samples = torch.zeros(
                num_samples * num_envs * joint_dof_count,
                dtype=torch.float32,
                device=model.adapter,
                requires_grad=True
            )
            ...
            for sample in range(num_samples):
                ...
                bundle_state = model.state()

                # Select environments using DFlex kernel
                tape.launch(
                    func=select_envs_joint_state,
                    dim=num_envs,
                    inputs=[
                        state_in.joint_q,
                        state_in.joint_qd,
                    ...
                    outputs=[
                        bundle_state.joint_q,
                        bundle_state.joint_qd,
            ...
            avg_bundle_joint_qd = torch.zeros(
                num_envs * joint_dof_count,
                dtype=torch.float32,
                device=model.adapter,
                requires_grad=True
            )
