...
class AnymalVelocityEnv(DFlexEnv):
    """
    Anymal velocity tracking environment
    """
    ...
    def __init__(
        ...
        super(AnymalVelocityEnv, self).__init__(
            num_envs,
            num_obs,
            num_act,
            episode_length,
        ...
        self.undesired_thigh_contact_links = [42, 52, 62, 72]  # LF_HAA, RF_HAA, LH_HAA, RH_HAA
    ...
    def init_sim(self):
        ...
        self.joint_limits = [
            (-0.72, 0.49),            # LF_HAA
            (-9.42477796077, 9.42477796077),  # LF_HFE
            (-9.42477796077, 9.42477796077),  # LF_KFE
            (-0.49, 0.72),            # RF_HAA
            (-9.42477796077, 9.42477796077),  # RF_HFE
            (-9.42477796077, 9.42477796077),  # RF_KFE
            (-0.72, 0.49),            # LH_HAA
            (-9.42477796077, 9.42477796077),  # LH_HFE
            (-9.42477796077, 9.42477796077),  # LH_KFE
            (-0.49, 0.72),            # RH_HAA
            (-9.42477796077, 9.42477796077),  # RH_HFE
            (-9.42477796077, 9.42477796077),  # RH_KFE
        ]
        ...
        # filename = "anymal_c_old/urdf/anymal_minimal.urdf"
    ...
    def set_act(self, action):
        ...
        #
        # # Set joint targets for position control (skip base pose: indices 0-6)
        # self.state.joint_target.view(self.num_envs, -1)[:, 6:] = target_joint_positions
        #
        # # Clear joint actuation to prevent torque control
        # self.state.joint_act.view(self.num_envs, -1)[:, 6:] = 0.0

        # TODO: Uncomment if above doesn't work, check dims though
        # Convert action to target joint positions
        target_joint_positions = action + self.default_joint_pos
    ...
    def calculate_reward(self, obs, act):
        ...
        air_time = torch.sum((feet_air_times - 0.5) * (feet_air_times > 0.0), dim=1) * (
                torch.norm(self._commands[:, [0, 2]], dim=1) > 0.1
        )
