...
class Go2VelocityEnv(DFlexEnv):
    ...
    def compute_termination(self, obs, act):
        ...
        invalid_value_mask = (torch.abs(joint_q[:, :6]) > 5).sum(-1) > 0
        invalid_value_mask = (
            invalid_value_mask | (torch.abs(joint_qd[:, :6]) > 5).sum(-1) > 0
        )

        invalid_value_mask = (
            invalid_value_mask | (torch.abs(joint_q[:, 6:]) > 10).sum(-1) > 0
        )

        invalid_value_mask = (
            invalid_value_mask | (torch.abs(joint_qd[:, 6:]) > 20).sum(-1) > 0
        )
        ...
        if self.wandb_enabled and termination.any():
            try:
                termination_reasons = {
                    ...
                    "termination/invalid_joint_q": (torch.abs(joint_q) > 20).sum().item(),
                    "termination/invalid_joint_qd": (torch.abs(joint_qd) > 20).sum().item(),
